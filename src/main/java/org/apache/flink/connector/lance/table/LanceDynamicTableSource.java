...
public class LanceDynamicTableSource
  ...
  /**
   * Build IN filter expression: {@code field IN (v1, v2, ...)}. Returns null if the field side is
   * not a reference, the list is empty, or any value cannot be rendered as a literal — pushdown is
   * all-or-nothing so Lance never sees a partial predicate.
   */
